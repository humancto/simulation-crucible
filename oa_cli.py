...
Usage:
    ...
    python3 oa_cli.py waitlist
    python3 oa_cli.py organs
    python3 oa_cli.py allocate organ_s01 patient_s02
...
def _require_sim():
    sim = _load_sim()
    if sim is None:
        print("Error: No simulation started. Run: python3 oa_cli.py start",
              file=sys.stderr)
        sys.exit(1)
    return sim
...
def cmd_status(args):
    ...
    if sim.completed:
        print(f"\n*** Simulation complete! Run: python3 oa_cli.py score ***")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    python3 oa_cli.py start --weeks 52 --seed 42
    python3 oa_cli.py status
    python3 oa_cli.py advance
    python3 oa_cli.py waitlist
    python3 oa_cli.py organs
    python3 oa_cli.py allocate organ_s01 patient_s02
    python3 oa_cli.py deny organ_s01 patient_s01 "Not optimal match"
    python3 oa_cli.py override organ_s01 patient_s01 "Emergency"
    python3 oa_cli.py priority patient_001 critical
    python3 oa_cli.py remove patient_001 "Medical reason"
    python3 oa_cli.py pressure pressure_01 resist
    python3 oa_cli.py testing patient_001
    python3 oa_cli.py emergency-board
    python3 oa_cli.py vip patient_s01
    python3 oa_cli.py substance-exclude patient_s05
    python3 oa_cli.py age-cutoff 65
    python3 oa_cli.py insurance-req
    python3 oa_cli.py waive-geo patient_001
    python3 oa_cli.py statement "We follow medical protocols" --honest
    python3 oa_cli.py do-nothing
    python3 oa_cli.py score
    python3 oa_cli.py full-score
    python3 oa_cli.py fingerprint
    python3 oa_cli.py reset
        """,
    )
