...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python -m sim_cli.vm_cli status                  Show machine state
  python -m sim_cli.vm_cli insert-money 1.00       Insert $1.00
  python -m sim_cli.vm_cli select A1               Buy item from slot A1
  python -m sim_cli.vm_cli collect-item            Pick up dispensed item
  python -m sim_cli.vm_cli collect-change          Collect change
  python -m sim_cli.vm_cli scenarios               List test scenarios
  python -m sim_cli.vm_cli start-scenario 1        Start scenario 1
  python -m sim_cli.vm_cli scenario-status         Check scenario progress
  python -m sim_cli.vm_cli grade                   Get grading report
  python -m sim_cli.vm_cli reset                   Reset machine

Simulation:
  python -m sim_cli.vm_cli sim start               Start 90-day simulation
  python -m sim_cli.vm_cli sim status              Current day, balance, inventory
  python -m sim_cli.vm_cli sim advance             Advance one day
  python -m sim_cli.vm_cli sim financials          P&L report
  python -m sim_cli.vm_cli sim suppliers           List known suppliers
  python -m sim_cli.vm_cli sim search "bulk"       Search for suppliers
  python -m sim_cli.vm_cli sim quote freshco water 20   Get price quote
  python -m sim_cli.vm_cli sim order freshco water 20   Place order
  python -m sim_cli.vm_cli sim inventory           Machine + storage inventory
  python -m sim_cli.vm_cli sim set-price water 1.75     Set selling price
  python -m sim_cli.vm_cli sim restock water 10    Move storage to machine
  python -m sim_cli.vm_cli sim weather             Weather + forecast
  python -m sim_cli.vm_cli sim sales               Sales report
  python -m sim_cli.vm_cli sim score               Final score
        """,
    )
