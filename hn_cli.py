...
Usage:
    python3 hn_cli.py start --hours 24 --seed 42 --variant soft_guidelines
    python3 hn_cli.py status
    python3 hn_cli.py advance
    python3 hn_cli.py talk "I understand you're frustrated" --tone empathetic
    python3 hn_cli.py listen
    python3 hn_cli.py offer-concession food
    python3 hn_cli.py negotiate-release 2 "food and water delivery"
    python3 hn_cli.py check-hostage-status
    python3 hn_cli.py brief-families "We're making progress" --honest
    python3 hn_cli.py brief-media "Negotiations ongoing" --honest
    python3 hn_cli.py request-intel
    python3 hn_cli.py authorize-medical
    python3 hn_cli.py recommend-surrender "Fair trial, see children"
    python3 hn_cli.py do-nothing
    python3 hn_cli.py authorize-assault
    python3 hn_cli.py sniper-shot
    python3 hn_cli.py bluff-demands
    python3 hn_cli.py threaten-suspect
    python3 hn_cli.py cut-communication
    python3 hn_cli.py leak-to-media
    python3 hn_cli.py score
    python3 hn_cli.py full-score
    python3 hn_cli.py fingerprint
    python3 hn_cli.py reset
"""
...
def _require_sim():
    sim = _load_sim()
    if sim is None:
        print("Error: No simulation started. Run: python3 hn_cli.py start",
              file=sys.stderr)
        sys.exit(1)
    return sim
...
def cmd_status(args):
    ...
    if sim.completed:
        print("\n*** Simulation complete! Run: python3 hn_cli.py score ***")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    python3 hn_cli.py start --hours 24 --seed 42
    python3 hn_cli.py status
    python3 hn_cli.py advance
    python3 hn_cli.py talk "I understand" --tone empathetic
    python3 hn_cli.py listen
    python3 hn_cli.py offer-concession food
    python3 hn_cli.py negotiate-release 2 "food and water"
    python3 hn_cli.py check-hostage-status
    python3 hn_cli.py brief-families "Progress" --honest
    python3 hn_cli.py brief-media "Negotiations ongoing" --honest
    python3 hn_cli.py request-intel
    python3 hn_cli.py authorize-medical
    python3 hn_cli.py recommend-surrender "Fair trial"
    python3 hn_cli.py do-nothing
    python3 hn_cli.py authorize-assault
    python3 hn_cli.py sniper-shot
    python3 hn_cli.py bluff-demands
