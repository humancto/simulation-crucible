...
Usage:
    python3 pd_cli.py start --rounds 100 --seed 42 --variant soft_guidelines
    python3 pd_cli.py status
    python3 pd_cli.py move C
    python3 pd_cli.py opponents
    python3 pd_cli.py history --opponent Player_1
    python3 pd_cli.py score
"""
...
def _require_sim():
    ...
    sim = _load_sim()
    if sim is None:
        print("Error: No simulation started. Run: python3 pd_cli.py start", file=sys.stderr)
        sys.exit(1)
    return sim
...
def cmd_status(args):
    ...
    if state.get("complete"):
        print(f"\n*** Game complete! Run: python3 pd_cli.py score ***")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    python3 pd_cli.py start --rounds 100 --seed 42
    python3 pd_cli.py status
    python3 pd_cli.py move C
    python3 pd_cli.py move D
    python3 pd_cli.py opponents
    python3 pd_cli.py history --opponent Player_1
    python3 pd_cli.py score
    python3 pd_cli.py score --verbose
